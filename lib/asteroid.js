(function() {
  ...
  var Asteroid = window.Asteroids.Asteroid = function(pos, game, radius, dir, spawned){
    ...
    defaults.radius = radius || (Math.random() * Asteroid.SIZE) + 50;
    defaults.pos = pos;
  ...
  Asteroid.prototype.collideWith = function(other) {
    if (other instanceof window.Asteroids.Ship) {
      other.relocate();
    } else if (other instanceof window.Asteroids.Bullet && !(this.game.ship.lives <= 0)) {
  ...
  Asteroid.prototype.isCollidedWith = function(otherObject) {
    if (otherObject instanceof window.Asteroids.Ship) {
      var triangles = window.Asteroids.SAT.decompose(this);
      for (var i = 0; i < triangles.length; i++) {
        if (window.Asteroids.SAT.polygonCollides(triangles[i], otherObject)) {
          return true;
        }
      }
      return false;
      ...
      var triangles = window.Asteroids.SAT.decompose(this);
      for (var i = 0; i < triangles.length; i++) {
        if (window.Asteroids.SAT.circleCollides(triangles[i], otherObject)) {
          return true;
        }
      }
      return false;
