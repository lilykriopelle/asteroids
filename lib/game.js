;(function(){
 ...
 Game.prototype.levelUp = function() {
  ...
  Game.prototype.renderLives = function(ctx) {
    ...
    ctx.lineWidth = 2;
    ctx.rect(Game.DIM_X - 170, 25, 160, 50);
    ctx.stroke();

    for (var j = 0; j < this.ship.lives; j++) {
      var lifePos = [(Game.DIM_X - 30 * j) - 20, 50];
      ctx.beginPath();
      ctx.moveTo(lifePos[0], lifePos[1]);
      ctx.lineTo(lifePos[0] - 20, lifePos[1] - 7);
      ctx.lineTo(lifePos[0] - 20, lifePos[1] + 7);
  ...
  Game.prototype.randomPosition = function() {
    return [
     Math.random() * (Game.DIM_X - 170),
     Math.random() * (Game.DIM_Y - 75)
   ];
 };
  ...
  Game.prototype.checkCollisions = function() {
    ...
    for (var i = 0; i < this.asteroids.length; i++) {
      var notAsteroids = this.bullets.concat(this.ship);
      for (var j = 0; j < notAsteroids.length; j++) {
        if (this.asteroids[i].isCollidedWith(notAsteroids[j])) {
          collidedPairs.push([this.asteroids[i], notAsteroids[j]]);
        }
      }
    }
  ...
  Game.prototype.step = function () {
    ...
    if (this.ship.lives <= 0) {
      $(".final-score").html(this.current_score);
      $(".lose-pop-up").addClass("active");
    }
  };

  Game.prototype.reset = function() {
    if (this.ship.lives <= 0 ) {
      $(".lose-pop-up").removeClass("active");
      this.current_score = 0;
